...
def _build_procurement_export_workbook(export_rows: list[dict], include_internal_notes: bool = False):
    ...
    def _add_logo(sheet_name: str):
        ...
        try:
            ...
            base_w = float(logo.width or 1.0)
            base_h = float(logo.height or 1.0)
            target_w = 220.0
            ratio = base_h / base_w if base_w > 0 else 0.45
            logo.width = int(target_w)
            logo.height = int(target_w * ratio)
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        detail_df.to_excel(writer, sheet_name="Order List", index=False, startrow=6)
        supplier_df.to_excel(writer, sheet_name="By Supplier", index=False, startrow=6)
        type_df.to_excel(writer, sheet_name="By Type", index=False, startrow=6)
        room_df.to_excel(writer, sheet_name="By Room", index=False, startrow=6)
        for sheet in ["Order List", "By Supplier", "By Type", "By Room"]:
            _add_logo(sheet)
